...
def main():
    # load the regular dataset and augmented dataset
    x = np.load("iris dataset/iris_features.npy")
    y = np.load("iris dataset/iris_labels.npy")
    N = 120
    x_train = x[:N]
    x_test = x[N:]
    y_train = y[:N]
    y_test = y[N:]
    xa_train = np.load("iris dataset/iris_train_features_augmented.npy")
    ya_train = np.load("iris dataset/iris_train_labels_augmented.npy")
    xa_test = np.load("iris dataset/iris_test_features_augmented.npy")
    ya_test = np.load("iris dataset/iris_test_labels_augmented.npy")
